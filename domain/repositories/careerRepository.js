...
class CareerRepository {
    ...
    async create(careerData) {
        const { name, curriculumUrl } = careerData;
        ...
        const { rows } = await db.query(
            'INSERT INTO careers (career_id, name, curriculum_url) VALUES ($1, $2, $3) RETURNING *',
            [tempCareerId, name, curriculumUrl]
        );
        return rows[0];
    }
    ...
    async update(id, careerData) {
        ...
        const { rows } = await db.query(
            ...
            'UPDATE careers SET name = $1, curriculum_url = $2 WHERE id = $3 RETURNING *',
            [name, curriculumUrl || null, id]
        );
        if (rows.length === 0) {
