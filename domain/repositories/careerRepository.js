...
class CareerRepository {
    ...
    async create(careerData) {
        const { name, curriculumUrl, description } = careerData;
        ...
        const { rows } = await db.query(
            'INSERT INTO careers (career_id, name, curriculum_url, description) VALUES ($1, $2, $3, $4) RETURNING *',
            [tempCareerId, name, curriculumUrl, description]
        );
        return rows[0];
    }
    ...
    async update(id, careerData) {
        ...
        const { rows } = await db.query(
            ...
            'UPDATE careers SET name = $1, curriculum_url = $2, description = $3 WHERE id = $4 RETURNING *',
            [name, curriculumUrl || null, description, id]
        );
        if (rows.length === 0) {
