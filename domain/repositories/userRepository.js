...
class UserRepository {
    ...
    async findByRole(role) {
        const { rows } = await db.query(`SELECT id, name, email, role FROM users WHERE role = $1 ORDER BY name`, [role]);
        return rows;
    }
    ...
    async create(email, password, name, role = 'student') {
        ...
        const queryText = 'INSERT INTO users(name, email, password_hash, role) VALUES($1, $2, $3, $4) RETURNING *';
    ...
    try {
        ...
        if (res.rows.length === 0) {
            ...
            await db.query(
                ...
                [name, adminEmail, adminPasswordHash, role]
            );
