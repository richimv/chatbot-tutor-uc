class AdminManager {
    ...
    displayInstructors() {
        ...
        // ✅ SOLUCIÓN: Pasar 'true' para que se muestre el botón de restablecer contraseña.
        content += this.allInstructors.map(instructor => createAdminItemCardHTML(instructor, 'instructor', `(${instructor.email})`, true)).join('');
        container.innerHTML = content;
    }
    ...
    displayStudents() {
        ...
        if (!this.allStudents || this.allStudents.length === 0) {
            container.innerHTML = `<div class="tab-header"><button class="btn-primary" onclick="adminManager.openGenericModal('student')">Añadir Alumno</button></div><p class="empty-state">No hay alumnos.</p>`;
            return;
        }
        let content = `<div class="tab-header"><button class="btn-primary" onclick="window.adminManager.openGenericModal('student')">Añadir Alumno</button></div>`;
        content += this.allStudents.map(student => createAdminItemCardHTML(student, 'student', `(${student.email})`, true)).join('');
        container.innerHTML = content;
    }
    ...
    openGenericModal(type, id = null) {
        ...
        // ✅ SOLUCIÓN DEFINITIVA: Convertir el ID de string a número ANTES de buscar.
        const numericId = id ? parseInt(id, 10) : null;
        let currentItem = null; // Se buscará con el ID numérico.
        ...
        switch (type) {
            ...
            case 'career':
                ...
                if (numericId) currentItem = this.allCareers.find(c => c.id === numericId);
            ...
            case 'course':
                ...
                if (numericId) currentItem = this.allCourses.find(c => c.id === numericId);
            ...
            case 'topic':
                ...
                if (numericId) currentItem = this.allTopics.find(t => t.id === numericId);
            ...
            case 'section':
                ...
                if (numericId) currentItem = this.allSections.find(s => s.id === numericId);
                fieldsHTML = `
                    <fieldset>
            ...
            case 'instructor':
                ...
                if (numericId) currentItem = this.allInstructors.find(i => i.id === numericId);
            ...
            case 'student': // ✅ NUEVO
                ...
                if (numericId) currentItem = this.allStudents.find(i => i.id === numericId);
            ...
            case 'book':
                ...
                if (numericId) currentItem = this.allBooks.find(b => b.id === numericId);
    ...
    createSearchableSelect(id, label, options, selectedValue, optional = false) {
        // ✅ SOLUCIÓN: Convertir el valor seleccionado a número para una comparación correcta.
        const numericSelectedValue = selectedValue ? parseInt(selectedValue, 10) : null;

        let optionsHTML = optional ? '<option value="">-- Ninguno --</option>' : '';
        optionsHTML += options.map(opt => `<option value="${opt.id}" ${opt.id === numericSelectedValue ? 'selected' : ''}>${opt.name}</option>`).join('');
        return `
    ...
    async handleResetPassword(userId) {
        ...
        if (!instructor) {
            alert('Error: No se encontró al instructor.');
            return;
        }

        if (!confirm(`¿Estás seguro de que quieres restablecer la contraseña para ${instructor.name}? Se generará una nueva contraseña temporal.`)) {
            return;
        }
        ...
        try {
            ...
            alert(`¡Éxito! La nueva contraseña temporal para ${instructor.name} es:\n\n${newPassword}\n\nPor favor, compártela de forma segura.`);
        } catch (error) {
